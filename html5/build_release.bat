...
%COMPPATH%\RenderClipComponent.cpp %COMPPATH%\ArcadeInputComponent.cpp %COMPPATH%\UnpackArchiveComponent.cpp %COMPPATH%\LogDisplayComponent.cpp %COMPPATH%\TouchDragComponent.cpp %COMPPATH%\RenderScissorComponent.cpp
...
SET CUSTOM_FLAGS=%CUSTOM_FLAGS% -O3 -D_DEBUG

REM -D_DEBUG

REM -s GL_UNSAFE_OPTS=0 -s WARN_ON_UNDEFINED_SYMBOLS=1 -s EXCEPTION_DEBUG=1 -s STACK_OVERFLOW_CHECK=2 -s ASSERTIONS=1 -s DEMANGLE_SUPPORT=1 -s ALIASING_FUNCTION_POINTERS=0
)
